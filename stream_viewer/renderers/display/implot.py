...
# Determine base class for ImPlotOpenGLWidget
_ImPlotBaseClass = QtWidgets.QWidget if QOpenGLWidget is None else QOpenGLWidget

class ImPlotOpenGLWidget(_ImPlotBaseClass):  # type: ignore
    ...
    def initializeGL(self):
        ...
        try:
            ...
            # Set up ImGui IO
            io = imgui.get_io()
            # Note: ConfigFlags may not be available in all slimgui versions
            # Viewports support is optional and may not be needed for basic rendering
            try:
                if hasattr(io, 'ConfigFlags') and hasattr(imgui, 'ConfigFlags'):
                    io.ConfigFlags |= imgui.ConfigFlags.VIEWPORTS_ENABLE  # type: ignore
            except (AttributeError, TypeError):
                # ConfigFlags not available, skip viewports configuration
                pass
